...
#Loading all the necessary libariers 
...
Displaying mean and standard deviation of all flights where month = 1 grouped by carrier in ascending order of the mean and standard deviation
...
#| message: false
#| warning: false
#Tibble with carriers along columns and average delays along the rows
...
#Proportion of flights that were delayed (dep_delay greater than 0) but arrived on or before time (arr_delay less than or equal to 0)
tibble2 = flights |> group_by(carrier,month) |> filter(dep_delay > 0) |> filter(arr_delay <= 0)

tibble2

#Assuming it should be as a proportion of all flights
...
```{r}
#All routes flown by more than one airline
distinctRoutes = flights |> distinct(origin,dest,carrier) |> count(origin,dest) |> filter(n>1)
distinctRoutes

#Routes along with their delays 
delays = flights |> select(origin,dest,carrier,arr_delay) 

#Finds average delays grouped by carrier and then joins that to the carrier names 
averageDelays = distinctRoutes |>  left_join(delays) |> group_by(carrier) |>  summarise(Average_Delay = mean(arr_delay,na.rm = T)) |>  left_join(airlines) 
averageDelays

#Finding best average delays for each route 
#Keep all ensures only first row is selected 
bestAve = distinctRoutes |>  left_join(delays) |> group_by(origin,dest,carrier) |>  summarise(Average_Delay = mean(arr_delay,na.rm = T)) |> arrange(origin,dest,Average_Delay) |> distinct(origin,dest, .keep_all = T)
bestAve

#Finding worst  average delays for each route 
#Keep all ensures only first row is selected 
worstAve = distinctRoutes |>  left_join(delays) |> group_by(origin,dest,carrier) |>  summarise(Average_Delay = mean(arr_delay,na.rm = T)) |> arrange(origin,dest,desc(Average_Delay)) |> distinct(origin,dest, .keep_all = T) 

#Renaming one of the columns in order to perform mutation 
worstAve = worstAve |> rename(Average_DelayWorst = Average_Delay)
worstAve

#Route with greatest difference 
bind_cols(bestAve,worstAve) |> mutate(diff = Average_DelayWorst-Average_Delay) |> arrange(desc(diff)) |> select(origin...1,dest...2,diff) |> filter(row_number() == 1)
...
#Using function to see irregularities in data 
UtilsDataRSV::view_cols(df)
```

Columns with missing entries are: glucose and height. The gender column has female incorrectly spelt as femal and disease status has both values Healthy and healthy some of it's columns.
